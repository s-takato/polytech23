%%% Title presen23103
\documentclass[landscape,10pt]{ujarticle}
\special{papersize=\the\paperwidth,\the\paperheight}
\usepackage{ketpic,ketlayer}
\usepackage{ketslide}
\usepackage{amsmath,amssymb}
\usepackage{bm,enumerate}
\usepackage[dvipdfmx]{graphicx}
\usepackage{color}
\definecolor{slidecolora}{cmyk}{0.98,0.13,0,0.43}
\definecolor{slidecolorb}{cmyk}{0.2,0,0,0}
\definecolor{slidecolorc}{cmyk}{0.2,0,0,0}
\definecolor{slidecolord}{cmyk}{0.2,0,0,0}
\definecolor{slidecolore}{cmyk}{0,0,0,0.5}
\definecolor{slidecolorf}{cmyk}{0,0,0,0.5}
\definecolor{slidecolori}{cmyk}{0.98,0.13,0,0.43}
\def\setthin#1{\def\thin{#1}}
\setthin{0}
\newcommand{\slidepage}[1][s]{%
\setcounter{ketpicctra}{18}%
\if#1m \setcounter{ketpicctra}{1}\fi
\hypersetup{linkcolor=black}%

\begin{layer}{118}{0}
\putnotee{122}{-\theketpicctra.05}{\small\thepage/\pageref{pageend}}
\end{layer}\hypersetup{linkcolor=blue}

}
\usepackage{emath}
\usepackage{emathEy}
\usepackage{emathMw}
\usepackage{pict2e}
\usepackage{ketlayermorewith2e}
\usepackage[dvipdfmx,colorlinks=true,linkcolor=blue,filecolor=blue]{hyperref}
\newcommand{\hiduke}{0508}
\newcommand{\hako}[2][1]{\fbox{\raisebox{#1mm}{\mbox{}}\raisebox{-#1mm}{\mbox{}}\,\phantom{#2}\,}}
\newcommand{\hakoa}[2][1]{\fbox{\raisebox{#1mm}{\mbox{}}\raisebox{-#1mm}{\mbox{}}\,#2\,}}
\newcommand{\hakom}[2][1]{\hako[#1]{$#2$}}
\newcommand{\hakoma}[2][1]{\hakoa[#1]{$#2$}}
\def\rad{\;\mathrm{rad}}
\def\deg#1{#1^{\circ}}
\newcommand{\sbunsuu}[2]{\scalebox{0.6}{$\bunsuu{#1}{#2}$}}
\def\pow{$\hspace{-1.5mm}＾\hspace{-1mm}$}
\def\dlim{\displaystyle\lim}
\newcommand{\brd}[2][1]{\scalebox{#1}{\color{red}\fbox{\color{black}$#2$}}}
\newcommand\down[1][0.5zw]{\vspace{#1}\\}
\newcommand{\sfrac}[3][0.65]{\scalebox{#1}{$\frac{#2}{#3}$}}
\newcommand{\phn}[1]{\phantom{#1}}
\newcommand{\scb}[2][0.6]{\scalebox{#1}{#2}}
\newcommand{\dsum}{\displaystyle\sum}
\def\pow{$\hspace{-1.5mm}＾\hspace{-1mm}$}
\def\dlim{\displaystyle\lim}
\def\dint{\displaystyle\int}

\setmargin{25}{145}{15}{100}

\ketslideinit

\pagestyle{empty}

\begin{document}

\begin{layer}{120}{0}
\putnotese{0}{0}{\input{fig/slide0.tex}}
\end{layer}

\def\mainslidetitley{22}
\def\ketcletter{slidecolora}
\def\ketcbox{slidecolorb}
\def\ketdbox{slidecolorc}
\def\ketcframe{slidecolord}
\def\ketcshadow{slidecolore}
\def\ketdshadow{slidecolorf}
\def\slidetitlex{6}
\def\slidetitlesize{1.3}
\def\mketcletter{slidecolori}
\def\mketcbox{yellow}
\def\mketdbox{yellow}
\def\mketcframe{yellow}
\def\mslidetitlex{62}
\def\mslidetitlesize{2}

\color{black}
\Large\bf\boldmath
\addtocounter{page}{-1}

\def\MARU{}
\renewcommand{\MARU}[1]{{\ooalign{\hfil$#1$\/\hfil\crcr\raise.167ex\hbox{\mathhexbox20D}}}}
\renewcommand{\slidepage}[1][s]{%
\setcounter{ketpicctra}{18}%
\if#1m \setcounter{ketpicctra}{1}\fi
\hypersetup{linkcolor=black}%
\begin{layer}{118}{0}
\putnotee{115}{-\theketpicctra.05}{\small\hiduke-\thepage/\pageref{pageend}}
\end{layer}\hypersetup{linkcolor=blue}
}
\newcounter{ban}
\setcounter{ban}{1}
\newcommand{\monban}[1][\hiduke]{%
#1-\theban\ %
\addtocounter{ban}{1}%
}
\newcommand{\monbannoadd}[1][\hiduke]{%
#1-\theban\ %
}
\newcommand{\addban}{%
\addtocounter{ban}{1}%
}
\newcounter{edawidth}
\newcounter{edactr}
\newcommand{\seteda}[1]{% 20220708 modified
\setcounter{edawidth}{#1}
\setcounter{edactr}{1}
}
\newcommand{\eda}[2][\theedawidth]{%
\Ltab{#1 mm}{[\theedactr]\ #2}%
\addtocounter{edactr}{1}%
}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%

\mainslide{2次関数と方程式(復習+）}


\slidepage[m]
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%

\newslide{2次関数のグラフ}

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\end{layer}

\begin{enumerate}[(1)]
\item
$y=a(x-b)^2+c$\\
\end{enumerate}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{0}{\scalebox{0.7}{\input{fig/presen22103-1.1.tex}}}
\end{layer}

\begin{enumerate}[(1)]
\item
$y=a(x-b)^2+c$\\
　・$y=ax^2$のグラフと形は同じ\\
\end{enumerate}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{0}{\scalebox{0.7}{\input{fig/presen22103-1.2.tex}}}
\end{layer}

\begin{enumerate}[(1)]
\item
$y=a(x-b)^2+c$\\
　・$y=ax^2$のグラフと形は同じ\\
　・$x$方向に$b$，$y$方向に$c$平行移動\\
\end{enumerate}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{0}{\scalebox{0.7}{\input{fig/presen22103-1.2.tex}}}
\end{layer}

\begin{enumerate}[(1)]
\item
$y=a(x-b)^2+c$\\
　・$y=ax^2$のグラフと形は同じ\\
　・$x$方向に$b$，$y$方向に$c$平行移動\\
　・頂点は $(b,\ c)$\vspace{-2mm}
\end{enumerate}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{0}{\scalebox{0.7}{\input{fig/presen22103-1.2.tex}}}
\end{layer}

\begin{enumerate}[(1)]
\item
$y=a(x-b)^2+c$\\
　・$y=ax^2$のグラフと形は同じ\\
　・$x$方向に$b$，$y$方向に$c$平行移動\\
　・頂点は $(b,\ c)$\vspace{-2mm}
\item
$y=ax^2+bx+c$\\
\end{enumerate}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{0}{\scalebox{0.7}{\input{fig/presen22103-1.2.tex}}}
\end{layer}

\begin{enumerate}[(1)]
\item
$y=a(x-b)^2+c$\\
　・$y=ax^2$のグラフと形は同じ\\
　・$x$方向に$b$，$y$方向に$c$平行移動\\
　・頂点は $(b,\ c)$\vspace{-2mm}
\item
$y=ax^2+bx+c$\\
　・(1)の形に変形（平方完成)\\
\end{enumerate}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{0}{\scalebox{0.7}{\input{fig/presen22103-1.2.tex}}}
\end{layer}

\begin{enumerate}[(1)]
\item
$y=a(x-b)^2+c$\\
　・$y=ax^2$のグラフと形は同じ\\
　・$x$方向に$b$，$y$方向に$c$平行移動\\
　・頂点は $(b,\ c)$\vspace{-2mm}
\item
$y=ax^2+bx+c$\\
　・(1)の形に変形（平方完成)\\
　・例）$y=x^2+4x+1$\\
\end{enumerate}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{0}{\scalebox{0.7}{\input{fig/presen22103-1.2.tex}}}
\end{layer}

\begin{enumerate}[(1)]
\item
$y=a(x-b)^2+c$\\
　・$y=ax^2$のグラフと形は同じ\\
　・$x$方向に$b$，$y$方向に$c$平行移動\\
　・頂点は $(b,\ c)$\vspace{-2mm}
\item
$y=ax^2+bx+c$\\
　・(1)の形に変形（平方完成)\\
　・例）$y=x^2+4x+1$\\
　　　　$\phantom{y}=(x^2+4x+4)-4+1$\\
\end{enumerate}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{0}{\scalebox{0.7}{\input{fig/presen22103-1.2.tex}}}
\end{layer}

\begin{enumerate}[(1)]
\item
$y=a(x-b)^2+c$\\
　・$y=ax^2$のグラフと形は同じ\\
　・$x$方向に$b$，$y$方向に$c$平行移動\\
　・頂点は $(b,\ c)$\vspace{-2mm}
\item
$y=ax^2+bx+c$\\
　・(1)の形に変形（平方完成)\\
　・例）$y=x^2+4x+1$\\
　　　　$\phantom{y}=(x^2+4x+4)-4+1=(x+2)^2-3$\\
\end{enumerate}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{0}{\scalebox{0.7}{\input{fig/presen22103-1.2.tex}}}
\end{layer}

\begin{enumerate}[(1)]
\item
$y=a(x-b)^2+c$\\
　・$y=ax^2$のグラフと形は同じ\\
　・$x$方向に$b$，$y$方向に$c$平行移動\\
　・頂点は $(b,\ c)$\vspace{-2mm}
\item
$y=ax^2+bx+c$\\
　・(1)の形に変形（平方完成)\\
　・例）$y=x^2+4x+1$\\
　　　　$\phantom{y}=(x^2+4x+4)-4+1=(x+2)^2-3$\\
　　　　頂点は $(-2,-3)$
\end{enumerate}

\newslide{2次方程式の解}

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\end{layer}

\begin{itemize}
\item
方程式$ax^2+bx+c=0$の解は\\
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{87}{-5}{\scalebox{0.7}{\input{fig/presen22103-2.tex}}}
\end{layer}

\begin{itemize}
\item
方程式$ax^2+bx+c=0$の解は\\
　$y=ax^2+bx+c$のグラフと\\
　$x$軸との交点の$x$座標\vspace{1zw}
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{87}{-5}{\scalebox{0.7}{\input{fig/presen22103-2.tex}}}
\end{layer}

\begin{itemize}
\item
方程式$ax^2+bx+c=0$の解は\\
　$y=ax^2+bx+c$のグラフと\\
　$x$軸との交点の$x$座標\vspace{1zw}
\item
[課題]\monban「関数のグラフ」でグラフをかき，\\
　　方程式の解（整数か分母が2の分数）を求めよ\seteda{110}\\
\eda{$y=x^2-2x-3,\ x^2-2x-3=0$}\\
\eda{$y=2x^2+7x-4,\ 2x^2+7x-4=0$}
\end{itemize}

\newslide{解の公式1}

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
$x^2+2ax+b=0$
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
$x^2+2ax+b=0$
\\$(x+a)^2-a^2+b=0$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
$x^2+2ax+b=0$
\\$(x+a)^2-a^2+b=0$
\\$(x+a)^2=a^2-b$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
$x^2+2ax+b=0$
\\$(x+a)^2-a^2+b=0$
\\$(x+a)^2=a^2-b$
\\$x+a=\pm \sqrt{a^2-b}$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
$x^2+2ax+b=0$
\\$(x+a)^2-a^2+b=0$
\\$(x+a)^2=a^2-b$
\\$x+a=\pm \sqrt{a^2-b}$
\vspace{2mm}\\よって　\fbox{\color{red}$x=-a\pm \sqrt{a^2-b}$}
\end{itemize}

\newslide{解の公式2}

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
2次方程式$ax^2+bx+c=0$の解\vspace{1mm}\\
\hspace*{3zw}\fbox{$x=\bunsuu{-b\pm\sqrt{b^2-4ac}}{2a}$}\vspace{-2mm}
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
2次方程式$ax^2+bx+c=0$の解\vspace{1mm}\\
\hspace*{3zw}\fbox{$x=\bunsuu{-b\pm\sqrt{b^2-4ac}}{2a}$}\vspace{-2mm}
\item
[]例）$2x^2-5x+1=0$\\
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
2次方程式$ax^2+bx+c=0$の解\vspace{1mm}\\
\hspace*{3zw}\fbox{$x=\bunsuu{-b\pm\sqrt{b^2-4ac}}{2a}$}\vspace{-2mm}
\item
[]例）$2x^2-5x+1=0$\\
\hspace*{3zw}$x=\bunsuu{5\pm \sqrt{5^2-4\cdot 2\cdot1}}{2\cdot 2}$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
2次方程式$ax^2+bx+c=0$の解\vspace{1mm}\\
\hspace*{3zw}\fbox{$x=\bunsuu{-b\pm\sqrt{b^2-4ac}}{2a}$}\vspace{-2mm}
\item
[]例）$2x^2-5x+1=0$\\
\hspace*{3zw}$x=\bunsuu{5\pm \sqrt{5^2-4\cdot 2\cdot1}}{2\cdot 2}$
$=\bunsuu{5\pm \sqrt{17}}{4}$\vspace{-2mm}
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
2次方程式$ax^2+bx+c=0$の解\vspace{1mm}\\
\hspace*{3zw}\fbox{$x=\bunsuu{-b\pm\sqrt{b^2-4ac}}{2a}$}\vspace{-2mm}
\item
[]例）$2x^2-5x+1=0$\\
\hspace*{3zw}$x=\bunsuu{5\pm \sqrt{5^2-4\cdot 2\cdot1}}{2\cdot 2}$
$=\bunsuu{5\pm \sqrt{17}}{4}$\vspace{-2mm}
\item
[課題] \monban 次の2次方程式を解け．\hfill Text P.74\seteda{55}\\
\eda{$2x^2+2x-3=0$}\eda{$3x^2+5x+1=0$}\\
\eda{$2x^2+x-2=0$}\eda{$x^2+3x+1=0$}
\end{itemize}

\mainslide{三角比}


\slidepage[m]
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%

\newslide{三平方の定理}

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotene{90}{35}{\input{fig/presen10310303.tex}}
\end{layer}

\begin{itemize}
\item
角$C$が直角の直角三角形$\triangle\mathrm{ABC}$
\item
$\mathrm{BC}=a,\ \mathrm{CA}=b,\ \mathrm{AB}=c$とおく
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotene{90}{35}{\input{fig/presen10310303.tex}}
\end{layer}

\begin{itemize}
\item
角$C$が直角の直角三角形$\triangle\mathrm{ABC}$
\item
$\mathrm{BC}=a,\ \mathrm{CA}=b,\ \mathrm{AB}=c$とおく
\item
[]\hspace*{0.5zw}$\Longrightarrow$\ \fbox{$a^2+b^2=c^2$}
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotene{90}{35}{\input{fig/presen10310303.tex}}
\putnotee{10}{57}{\input{fig/presen10310304a.tex}}
\putnotee{50}{60}{\input{fig/presen10310304b.tex}}
\putnotee{80}{55}{\input{fig/presen10310304c.tex}}
\end{layer}

\begin{itemize}
\item
角$C$が直角の直角三角形$\triangle\mathrm{ABC}$
\item
$\mathrm{BC}=a,\ \mathrm{CA}=b,\ \mathrm{AB}=c$とおく
\item
[]\hspace*{0.5zw}$\Longrightarrow$\ \fbox{$a^2+b^2=c^2$}
\item
[課題]\monbannoadd 図の$a,\ b,\ c$を求めよ\seteda{40}\\
\eda{}\eda[30]{}\eda{}
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotene{90}{35}{\input{fig/presen10310303.tex}}
\putnotee{10}{57}{\input{fig/presen10310304a.tex}}
\putnotee{50}{60}{\input{fig/presen10310304b.tex}}
\putnotee{80}{55}{\input{fig/presen10310304c.tex}}
\putnotee{80}{60}{\input{fig/presen10310304d.tex}}
\end{layer}

\begin{itemize}
\item
角$C$が直角の直角三角形$\triangle\mathrm{ABC}$
\item
$\mathrm{BC}=a,\ \mathrm{CA}=b,\ \mathrm{AB}=c$とおく
\item
[]\hspace*{0.5zw}$\Longrightarrow$\ \fbox{$a^2+b^2=c^2$}
\item
[課題]\monban 図の$a,\ b,\ c$を求めよ\seteda{40}\\
\eda{}\eda[30]{}\eda{}
\end{itemize}

\newslide{鋭角の三角比}

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{65}{-70}{\input{fig/sankakuhidef.tex}}
\end{layer}

\begin{itemize}
\item
[]$\cos\theta=\bunsuu{\mbox{底辺}}{\mbox{斜辺}}$
\item
[]$\sin\theta=\bunsuu{\mbox{高さ}}{\mbox{斜辺}}$
\item
[]$\tan\theta=\bunsuu{\mbox{高さ}}{\mbox{底辺}}$
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{65}{-70}{\input{fig/sankakuhidef.tex}}
\putnotese{65}{24}{\color{red}比だから大きさによらない}
\end{layer}

\begin{itemize}
\item
[]$\cos\theta=\bunsuu{\mbox{底辺}}{\mbox{斜辺}}$
\item
[]$\sin\theta=\bunsuu{\mbox{高さ}}{\mbox{斜辺}}$
\item
[]$\tan\theta=\bunsuu{\mbox{高さ}}{\mbox{底辺}}$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{65}{-70}{\input{fig/sankakuhidef.tex}}
\putnotese{65}{24}{\color{red}比だから大きさによらない}
\putnotese{75}{31}{\color{red}角$\theta$だけで決まる}
\end{layer}

\begin{itemize}
\item
[]$\cos\theta=\bunsuu{\mbox{底辺}}{\mbox{斜辺}}$
\item
[]$\sin\theta=\bunsuu{\mbox{高さ}}{\mbox{斜辺}}$
\item
[]$\tan\theta=\bunsuu{\mbox{高さ}}{\mbox{底辺}}$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{65}{-70}{\input{fig/sankakuhidef.tex}}
\putnotese{65}{24}{\color{red}比だから大きさによらない}
\putnotese{75}{31}{\color{red}角$\theta$だけで決まる}
\end{layer}

\begin{itemize}
\item
[]$\cos\theta=\bunsuu{\mbox{底辺}}{\mbox{斜辺}}$ {\color{blue}底辺}
\item
[]$\sin\theta=\bunsuu{\mbox{高さ}}{\mbox{斜辺}}$ {\color{blue}高さ}
\item
[]$\tan\theta=\bunsuu{\mbox{高さ}}{\mbox{底辺}}$ {\color{blue}比}
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{65}{-70}{\input{fig/sankakuhidef.tex}}
\putnotese{65}{24}{\color{red}比だから大きさによらない}
\putnotese{75}{31}{\color{red}角$\theta$だけで決まる}
\putnotese{75}{48}{\input{fig/trig3045.tex}}
\end{layer}

\begin{itemize}
\item
[]$\cos\theta=\bunsuu{\mbox{底辺}}{\mbox{斜辺}}$ {\color{blue}底辺}
\item
[]$\sin\theta=\bunsuu{\mbox{高さ}}{\mbox{斜辺}}$ {\color{blue}高さ}
\item
[]$\tan\theta=\bunsuu{\mbox{高さ}}{\mbox{底辺}}$ {\color{blue}比}
\item
[課題 ]\monban 次の三角比を求めよ．\seteda{34}\\
\eda{$\cos\deg{30}$}\eda{$\sin\deg{45}$}\\
\eda{$\tan\deg{60}$}
\end{itemize}

\newslide{練習(鋭角の三角比)}

\vspace*{18mm}

%%repeat=4
\slidepage

\begin{layer}{120}{0}
\putnotese{80}{10}{\input{fig/acutetrig.tex}}
\end{layer}

\begin{itemize}
\item
[課題]\monban 図の三角形について次を求めよ．\seteda{38}\\
\eda{$x\degree$}\eda{辺AC}\\
\eda{$\tan x$}\eda{$\cos x$}\\
\eda{$\sin x$}\eda{$\tan 59\degree$}\\
\eda{$\cos 59\degree$}\eda{$\sin 59\degree$}
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%

\newslide{三角比の拡張}

\vspace*{18mm}

\slidepage
\down
鋭角から以下の角に拡張する．
\begin{enumerate}[(1)]
\item
$0\degree$
\item
$90\degree$
\item
鈍角($90\degree<\theta<180\degree$)
\end{enumerate}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage
\down
鋭角から以下の角に拡張する．
\begin{enumerate}[(1)]
\item
$0\degree$
\item
$90\degree$
\item
鈍角($90\degree<\theta<180\degree$)
\item
[課題]\monban 「鈍角等の三角比」を動かそう．
次の三角比はどうなるだろうか．\seteda{35}\\
\eda{$\cos0\degree$}\eda{$\cos 90\degree$}
\eda{$\cos 120\degree$}
\end{enumerate}

\newslide{鈍角等の三角比}

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{80}{15}{\scalebox{0.5}{\input{fig/trigratiodonkakudef.tex}}}
\end{layer}

\begin{itemize}
\item
{\color{red}鈍角のとき，$\theta$を1つの角とする直角三角形ができない}
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{80}{15}{\scalebox{0.5}{\input{fig/trigratiodonkakudef.tex}}}
\end{layer}

\begin{itemize}
\item
{\color{red}鈍角のとき，$\theta$を1つの角とする直角三角形ができない}
\item
座標軸をおく
\item
頂点Pの座標を$(x,y)$とする
\item
$\mbox{斜辺}=\mathrm{OP},\mbox{底辺}=x,\mbox{高さ}=y$\vspace{1mm}\\
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{80}{15}{\scalebox{0.5}{\input{fig/trigratiodonkakudef.tex}}}
\end{layer}

\begin{itemize}
\item
{\color{red}鈍角のとき，$\theta$を1つの角とする直角三角形ができない}
\item
座標軸をおく
\item
頂点Pの座標を$(x,y)$とする
\item
$\mbox{斜辺}=\mathrm{OP},\mbox{底辺}=x,\mbox{高さ}=y$\vspace{1mm}\\
　$\cos\theta=\bunsuu{x}{\mathrm{OP}},\ \sin\theta=\bunsuu{y}{\mathrm{OP}},\ \tan\theta=\bunsuu{y}{x}$
\end{itemize}

\newslide{$\deg{0}$の三角比}

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{55}{5}{\includegraphics[bb=0.00 0.00 809.00 497.00,width=70mm]{fig/cos0.pdf}}
\end{layer}

\begin{itemize}
\item
[]$\cos\deg{0}=$
\item
[]$\sin\deg{0}=$
\item
[]$\tan\deg{0}=$
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{55}{5}{\includegraphics[bb=0.00 0.00 809.00 497.00,width=70mm]{fig/cos0.pdf}}
\end{layer}

\begin{itemize}
\item
[]$\cos\deg{0}=1$
\item
[]$\sin\deg{0}=$
\item
[]$\tan\deg{0}=$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{55}{5}{\includegraphics[bb=0.00 0.00 809.00 497.00,width=70mm]{fig/cos0.pdf}}
\end{layer}

\begin{itemize}
\item
[]$\cos\deg{0}=1$
\item
[]$\sin\deg{0}=0$
\item
[]$\tan\deg{0}=$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{55}{5}{\includegraphics[bb=0.00 0.00 809.00 497.00,width=70mm]{fig/cos0.pdf}}
\end{layer}

\begin{itemize}
\item
[]$\cos\deg{0}=1$
\item
[]$\sin\deg{0}=0$
\item
[]$\tan\deg{0}=0$
\end{itemize}

\newslide{$\deg{90}$の三角比}

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{60}{0}{\includegraphics[bb=0.00 0.00 663.00 504.00,width=50mm]{fig/cos90.pdf}}
\end{layer}

\begin{itemize}
\item
[]$\cos\deg{90}=$
\item
[]$\sin\deg{90}=$
\item
[]$\tan\deg{90}=$
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{60}{0}{\includegraphics[bb=0.00 0.00 663.00 504.00,width=50mm]{fig/cos90.pdf}}
\end{layer}

\begin{itemize}
\item
[]$\cos\deg{90}=0$
\item
[]$\sin\deg{90}=$
\item
[]$\tan\deg{90}=$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{60}{0}{\includegraphics[bb=0.00 0.00 663.00 504.00,width=50mm]{fig/cos90.pdf}}
\end{layer}

\begin{itemize}
\item
[]$\cos\deg{90}=0$
\item
[]$\sin\deg{90}=1$
\item
[]$\tan\deg{90}=$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{60}{0}{\includegraphics[bb=0.00 0.00 663.00 504.00,width=50mm]{fig/cos90.pdf}}
\end{layer}

\begin{itemize}
\item
[]$\cos\deg{90}=0$
\item
[]$\sin\deg{90}=1$
\item
[]$\tan\deg{90}=\mbox{値がない}$\vspace{1,5zw}
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{60}{0}{\includegraphics[bb=0.00 0.00 663.00 504.00,width=50mm]{fig/cos90.pdf}}
\end{layer}

\begin{itemize}
\item
[]$\cos\deg{90}=0$
\item
[]$\sin\deg{90}=1$
\item
[]$\tan\deg{90}=\mbox{値がない}$\vspace{1,5zw}
\item
[課題]\monban $\bunsuu{0}{0},\ \bunsuu{1}{0}$の値はどうなるか，次から選べ\vspace{2mm}\\
　 1　 0　 値がない　 値が決まらない\\
\end{itemize}

\newslide{鈍角の三角比の符号}

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{55}{10}{\includegraphics[bb=0.00 0.00 732.00 465.00,width=70mm]{fig/cosdonkakufugou.pdf}}
\end{layer}

\begin{itemize}
\item
[]$\cos$は
\item
[]$\sin$は
\item
[]$\tan$は
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{55}{10}{\includegraphics[bb=0.00 0.00 732.00 465.00,width=70mm]{fig/cosdonkakufugou.pdf}}
\end{layer}

\begin{itemize}
\item
[]$\cos$は$-$
\item
[]$\sin$は
\item
[]$\tan$は
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{55}{10}{\includegraphics[bb=0.00 0.00 732.00 465.00,width=70mm]{fig/cosdonkakufugou.pdf}}
\end{layer}

\begin{itemize}
\item
[]$\cos$は$-$
\item
[]$\sin$は$+$
\item
[]$\tan$は
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{55}{10}{\includegraphics[bb=0.00 0.00 732.00 465.00,width=70mm]{fig/cosdonkakufugou.pdf}}
\end{layer}

\begin{itemize}
\item
[]$\cos$は$-$
\item
[]$\sin$は$+$
\item
[]$\tan$は$-$
\end{itemize}

\newslide{三角比の相互関係}

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{6}{\input{fig/presen10310305.tex}}
\end{layer}

\begin{itemize}
\item
[(1)\ ]$\tan\theta=\bunsuu{\sin\theta}{\cos\theta}$
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{6}{\input{fig/presen10310305.tex}}
\end{layer}

\begin{itemize}
\item
[(1)\ ]$\tan\theta=\bunsuu{\sin\theta}{\cos\theta}$
\item
[\color{blue}証)]{\color{blue}$\tan\theta=\bunsuu{y}{x}=\bunsuu{\frac{y}{\mathrm{OP}}}{\frac{x}{\mathrm{OP}}}$}
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{6}{\input{fig/presen10310305.tex}}
\end{layer}

\begin{itemize}
\item
[(1)\ ]$\tan\theta=\bunsuu{\sin\theta}{\cos\theta}$
\item
[\color{blue}証)]{\color{blue}$\tan\theta=\bunsuu{y}{x}=\bunsuu{\frac{y}{\mathrm{OP}}}{\frac{x}{\mathrm{OP}}}$}
{\color{blue}$=\bunsuu{\sin\theta}{\cos\theta}$}
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{6}{\input{fig/presen10310305.tex}}
\end{layer}

\begin{itemize}
\item
[(1)\ ]$\tan\theta=\bunsuu{\sin\theta}{\cos\theta}$
\item
[\color{blue}証)]{\color{blue}$\tan\theta=\bunsuu{y}{x}=\bunsuu{\frac{y}{\mathrm{OP}}}{\frac{x}{\mathrm{OP}}}$}
{\color{blue}$=\bunsuu{\sin\theta}{\cos\theta}$}
\item
[(2)\ ]$\cos^2\theta+\sin^2\theta=1$\vspace{1.5zw}
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotee{60}{40}{\color{red}$\bigl(\cos\theta\bigr)^2$を$\cos^2 \theta$と書く}
\putnotee{60}{48}{\color{red}KeTMathではcos(2,$\theta$)}
\putnotese{90}{6}{\input{fig/presen10310305.tex}}
\end{layer}

\begin{itemize}
\item
[(1)\ ]$\tan\theta=\bunsuu{\sin\theta}{\cos\theta}$
\item
[\color{blue}証)]{\color{blue}$\tan\theta=\bunsuu{y}{x}=\bunsuu{\frac{y}{\mathrm{OP}}}{\frac{x}{\mathrm{OP}}}$}
{\color{blue}$=\bunsuu{\sin\theta}{\cos\theta}$}
\item
[(2)\ ]$\cos^2\theta+\sin^2\theta=1$\vspace{1.5zw}
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotee{60}{40}{\color{red}$\bigl(\cos\theta\bigr)^2$を$\cos^2 \theta$と書く}
\putnotee{60}{48}{\color{red}KeTMathではcos(2,$\theta$)}
\putnotese{90}{6}{\input{fig/presen10310305.tex}}
\end{layer}

\begin{itemize}
\item
[(1)\ ]$\tan\theta=\bunsuu{\sin\theta}{\cos\theta}$
\item
[\color{blue}証)]{\color{blue}$\tan\theta=\bunsuu{y}{x}=\bunsuu{\frac{y}{\mathrm{OP}}}{\frac{x}{\mathrm{OP}}}$}
{\color{blue}$=\bunsuu{\sin\theta}{\cos\theta}$}
\item
[(2)\ ]$\cos^2\theta+\sin^2\theta=1$\vspace{1.5zw}
\item
[\color{blue}証)]{\color{blue}$\cos^2\theta+\sin^2\theta=\bunsuu{x^2}{\mathrm{OP}^2}+\bunsuu{y^2}{\mathrm{OP}^2}$}
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotee{60}{40}{\color{red}$\bigl(\cos\theta\bigr)^2$を$\cos^2 \theta$と書く}
\putnotee{60}{48}{\color{red}KeTMathではcos(2,$\theta$)}
\putnotese{90}{6}{\input{fig/presen10310305.tex}}
\end{layer}

\begin{itemize}
\item
[(1)\ ]$\tan\theta=\bunsuu{\sin\theta}{\cos\theta}$
\item
[\color{blue}証)]{\color{blue}$\tan\theta=\bunsuu{y}{x}=\bunsuu{\frac{y}{\mathrm{OP}}}{\frac{x}{\mathrm{OP}}}$}
{\color{blue}$=\bunsuu{\sin\theta}{\cos\theta}$}
\item
[(2)\ ]$\cos^2\theta+\sin^2\theta=1$\vspace{1.5zw}
\item
[\color{blue}証)]{\color{blue}$\cos^2\theta+\sin^2\theta=\bunsuu{x^2}{\mathrm{OP}^2}+\bunsuu{y^2}{\mathrm{OP}^2}$}
{\color{blue}$=\bunsuu{x^2+y^2}{\mathrm{OP^2}}=1$}
\end{itemize}

\newslide{三角比の相互関係(問)}

\vspace*{18mm}

\slidepage
\vspace{4mm}

\begin{itemize}
\item
[例題]$\cos\theta=\bunsuu{1}{3}$のとき，$\sin\theta$を求めよ．
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage
\vspace{4mm}

\begin{itemize}
\item
[例題]$\cos\theta=\bunsuu{1}{3}$のとき，$\sin\theta$を求めよ．
\item
[解]$\bunsuu{1}{9}+\sin^2\theta=1$より　%
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\vspace{4mm}

\begin{itemize}
\item
[例題]$\cos\theta=\bunsuu{1}{3}$のとき，$\sin\theta$を求めよ．
\item
[解]$\bunsuu{1}{9}+\sin^2\theta=1$より　%
$\sin^2\theta=1-\bunsuu{1}{9}=\bunsuu{8}{9}$\\
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\vspace{4mm}

\begin{itemize}
\item
[例題]$\cos\theta=\bunsuu{1}{3}$のとき，$\sin\theta$を求めよ．
\item
[解]$\bunsuu{1}{9}+\sin^2\theta=1$より　%
$\sin^2\theta=1-\bunsuu{1}{9}=\bunsuu{8}{9}$\\
鋭角でも鈍角でも$\sin\theta>0$だから　\vphantom{$\bunsuu{2\sqrt{2}}{3}$}%
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\vspace{4mm}

\begin{itemize}
\item
[例題]$\cos\theta=\bunsuu{1}{3}$のとき，$\sin\theta$を求めよ．
\item
[解]$\bunsuu{1}{9}+\sin^2\theta=1$より　%
$\sin^2\theta=1-\bunsuu{1}{9}=\bunsuu{8}{9}$\\
鋭角でも鈍角でも$\sin\theta>0$だから　\vphantom{$\bunsuu{2\sqrt{2}}{3}$}%
$\sin\theta=\bunsuu{2\sqrt{2}}{3}$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\vspace{4mm}

\begin{itemize}
\item
[例題]$\cos\theta=\bunsuu{1}{3}$のとき，$\sin\theta$を求めよ．
\item
[解]$\bunsuu{1}{9}+\sin^2\theta=1$より　%
$\sin^2\theta=1-\bunsuu{1}{9}=\bunsuu{8}{9}$\\
鋭角でも鈍角でも$\sin\theta>0$だから　\vphantom{$\bunsuu{2\sqrt{2}}{3}$}%
$\sin\theta=\bunsuu{2\sqrt{2}}{3}$
\item
[課題]\monban $\sin\theta=\bunsuu{2}{3}$とする．次の場合のそれぞれについて$\cos\theta$を求めよ\vspace{2mm}\seteda{55}\\
\eda{$\theta$が鋭角のとき}\eda{$\theta$が鈍角のとき}
\end{itemize}

\mainslide{一般角}


\slidepage[m]
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%

\newslide{一般角}

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\end{layer}

\begin{itemize}
\item
これまで，角$\theta$は2つの線分の間の角だった\\
\hspace*{2zw}$0^{\circ} \leqq \theta \leqq 360^{\circ}$
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{78}{17}{\input{fig/generalangle200.tex}}
\end{layer}

\begin{itemize}
\item
これまで，角$\theta$は2つの線分の間の角だった\\
\hspace*{2zw}$0^{\circ} \leqq \theta \leqq 360^{\circ}$
\item
角を回転を表す量とすると\\
$\theta$はどんな実数でもよい．
\begin{itemize}
\item
[$\cdot$]$x$軸を始線とする
\item
[$\cdot$]$\theta >0^{\circ}$のとき，反時計回り
\end{itemize}
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{78}{17}{\input{fig/generalanglen30.tex}}
\end{layer}

\begin{itemize}
\item
これまで，角$\theta$は2つの線分の間の角だった\\
\hspace*{2zw}$0^{\circ} \leqq \theta \leqq 360^{\circ}$
\item
角を回転を表す量とすると\\
$\theta$はどんな実数でもよい．
\begin{itemize}
\item
[$\cdot$]$x$軸を始線とする
\item
[$\cdot$]$\theta >0^{\circ}$のとき，反時計回り
\item
[$\cdot$]$\theta<0^{\circ}$のとき，時計回り
\end{itemize}
\end{itemize}

\newslide{一般角}

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\end{layer}

\vspace*{5mm}

\noindent
「一般角」で一般角を見てみよう
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{85}{20}{\scalebox{0.7}{\input{fig/presen103xy.tex}}}
\putnotec{115}{30}{\normalsize\color{red}第１象限}
\putnotec{95}{30}{\normalsize\color{red}第２象限}
\putnotec{95}{52}{\normalsize\color{red}第３象限}
\putnotec{115}{52}{\normalsize\color{red}第４象限}
\end{layer}

\vspace*{5mm}

\noindent
「一般角」で一般角を見てみよう
\begin{itemize}
\item
[課題]\monban 次の角は第何象限にあるか．\seteda{40}\\
\eda{$\deg{400}$}\eda{$\deg{600}$}\\
\eda{$\deg{-500}$}\eda{$\deg{-700}$}
\end{itemize}

\newslide{一般角の三角比}

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\end{layer}

\begin{itemize}
\item
座標を使う（鈍角の場合と同じ）\vspace{-2mm}
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{75}{15}{\input{fig/generalangle240.tex}}
\end{layer}

\begin{itemize}
\item
座標を使う（鈍角の場合と同じ）\vspace{-2mm}
\item
[例]$\theta=240\degree$\vspace{-2mm}\\
\hspace*{1zw}$\cos\theta=\vphantom{\bunsuu{1}{2}}$\\
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{75}{15}{\input{fig/generalangle240.tex}}
\end{layer}

\begin{itemize}
\item
座標を使う（鈍角の場合と同じ）\vspace{-2mm}
\item
[例]$\theta=240\degree$\vspace{-2mm}\\
\hspace*{1zw}$\cos\theta=\bunsuu{-1}{2}=$\\
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{75}{15}{\input{fig/generalangle240.tex}}
\end{layer}

\begin{itemize}
\item
座標を使う（鈍角の場合と同じ）\vspace{-2mm}
\item
[例]$\theta=240\degree$\vspace{-2mm}\\
\hspace*{1zw}$\cos\theta=\bunsuu{-1}{2}=-\bunsuu{1}{2}$\\
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{75}{15}{\input{fig/generalangle240.tex}}
\end{layer}

\begin{itemize}
\item
座標を使う（鈍角の場合と同じ）\vspace{-2mm}
\item
[例]$\theta=240\degree$\vspace{-2mm}\\
\hspace*{1zw}$\cos\theta=\bunsuu{-1}{2}=-\bunsuu{1}{2}$\\
\hspace*{1zw}$\sin\theta=\vphantom{\bunsuu{\sqrt{3}}{2}}$\\
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{75}{15}{\input{fig/generalanglen30.tex}}
\end{layer}

\begin{itemize}
\item
座標を使う（鈍角の場合と同じ）\vspace{-2mm}
\item
[例]$\theta=240\degree$\vspace{-2mm}\\
\hspace*{1zw}$\cos\theta=\bunsuu{-1}{2}=-\bunsuu{1}{2}$\\
\hspace*{1zw}$\sin\theta=\bunsuu{\sqrt{3}}{2}$\\
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{75}{15}{\input{fig/generalanglen30.tex}}
\end{layer}

\begin{itemize}
\item
座標を使う（鈍角の場合と同じ）\vspace{-2mm}
\item
[例]$\theta=240\degree$\vspace{-2mm}\\
\hspace*{1zw}$\cos\theta=\bunsuu{-1}{2}=-\bunsuu{1}{2}$\\
\hspace*{1zw}$\sin\theta=\bunsuu{\sqrt{3}}{2}$\\
\hspace*{1zw}$\tan\theta=\vphantom{\bunsuu{\sqrt{3}}{2}}$\\
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{75}{15}{\input{fig/generalanglen30.tex}}
\end{layer}

\begin{itemize}
\item
座標を使う（鈍角の場合と同じ）\vspace{-2mm}
\item
[例]$\theta=240\degree$\vspace{-2mm}\\
\hspace*{1zw}$\cos\theta=\bunsuu{-1}{2}=-\bunsuu{1}{2}$\\
\hspace*{1zw}$\sin\theta=\bunsuu{\sqrt{3}}{2}$\\
\hspace*{1zw}$\tan\theta=\bunsuu{-\sqrt{3}}{-1}$\\
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{75}{15}{\input{fig/generalanglen30.tex}}
\end{layer}

\begin{itemize}
\item
座標を使う（鈍角の場合と同じ）\vspace{-2mm}
\item
[例]$\theta=240\degree$\vspace{-2mm}\\
\hspace*{1zw}$\cos\theta=\bunsuu{-1}{2}=-\bunsuu{1}{2}$\\
\hspace*{1zw}$\sin\theta=\bunsuu{\sqrt{3}}{2}$\\
\hspace*{1zw}$\tan\theta=\bunsuu{-\sqrt{3}}{-1}=\sqrt{3}$\vspace{-2mm}\\
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{75}{15}{\input{fig/generalanglen30.tex}}
\end{layer}

\begin{itemize}
\item
座標を使う（鈍角の場合と同じ）\vspace{-2mm}
\item
[例]$\theta=240\degree$\vspace{-2mm}\\
\hspace*{1zw}$\cos\theta=\bunsuu{-1}{2}=-\bunsuu{1}{2}$\\
\hspace*{1zw}$\sin\theta=\bunsuu{\sqrt{3}}{2}$\\
\hspace*{1zw}$\tan\theta=\bunsuu{-\sqrt{3}}{-1}=\sqrt{3}$\vspace{-2mm}\\
\item
[課題]\monban 次の値を求めよ．\seteda{35}\\
\eda{$\cos(-30\degree)$}\eda{$\sin(-30\degree)$}\eda{$\tan(-30\degree)$}
\end{itemize}

\mainslide{弧度法（radian）}


\slidepage[m]
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%

\newslide{角度の測り方1}

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{10}{\input{fig/kakudo60.tex}}
\end{layer}

\noindent
度　${}^{\circ}$\vspace{-1mm}
\begin{itemize}
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
１周を$360^{\circ}$とする
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
半周は$180^{\circ}$とする
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
一周の$\bunsuu{1}{360}$を$1^{\circ}$とする
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
数学的な意味は余りない
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
日常的には使いやすい
}%
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{10}{\input{fig/kakudo360.tex}}
\end{layer}

\noindent
度　${}^{\circ}$\vspace{-1mm}
\begin{itemize}
\item
１周を$360^{\circ}$とする
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
半周は$180^{\circ}$とする
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
一周の$\bunsuu{1}{360}$を$1^{\circ}$とする
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
数学的な意味は余りない
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
日常的には使いやすい
}%
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{10}{\input{fig/kakudo180.tex}}
\end{layer}

\noindent
度　${}^{\circ}$\vspace{-1mm}
\begin{itemize}
\item
１周を$360^{\circ}$とする
\item
半周は$180^{\circ}$とする
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
一周の$\bunsuu{1}{360}$を$1^{\circ}$とする
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
数学的な意味は余りない
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
日常的には使いやすい
}%
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{10}{\input{fig/kakudo001.tex}}
\end{layer}

\noindent
度　${}^{\circ}$\vspace{-1mm}
\begin{itemize}
\item
１周を$360^{\circ}$とする
\item
半周は$180^{\circ}$とする
\item
一周の$\bunsuu{1}{360}$を$1^{\circ}$とする
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
数学的な意味は余りない
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
日常的には使いやすい
}%
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage

\begin{layer}{120}{0}
\putnotese{90}{10}{\input{fig/kakudo001.tex}}
\end{layer}

\noindent
度　${}^{\circ}$\vspace{-1mm}
\begin{itemize}
\item
１周を$360^{\circ}$とする
\item
半周は$180^{\circ}$とする
\item
一周の$\bunsuu{1}{360}$を$1^{\circ}$とする
\item
数学的な意味は余りない
\item
日常的には使いやすい
\end{itemize}

\newslide{角度の測り方2（弧度法）}

\vspace*{18mm}

\hypertarget{para1pg1}{}

\begin{layer}{120}{0}
\putnotew{96}{73}{\hyperlink{para0pg5}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut||\!\lhd$}}}}}
\putnotew{101}{73}{\hyperlink{para1pg1}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut|\!\lhd$}}}}}
\putnotew{108}{73}{\hyperlink{para0pg5}{\fbox{\Ctab{4.5mm}{\scalebox{1}{\scriptsize $\mathstrut\!\!\lhd\!\!$}}}}}
\putnotew{115}{73}{\hyperlink{para1pg2}{\fbox{\Ctab{4.5mm}{\scalebox{1}{\scriptsize $\mathstrut\!\rhd\!$}}}}}
\putnotew{120}{73}{\hyperlink{para1pg5}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut \!\rhd\!\!|$}}}}}
\putnotew{125}{73}{\hyperlink{para2pg1}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut \!\rhd\!\!||$}}}}}
\putnotee{126}{73}{\scriptsize\color{blue} 1/5}
\end{layer}

\slidepage

\begin{layer}{120}{0}
\putnotese{87}{10}{\input{fig/radian.tex}}
\end{layer}

\begin{itemize}
\item
弧の長さ$\ell$と半径$r$の比\ $\theta(\rad)=\bunsuu{\ell}{r}$\vspace{-1mm}
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
半径$r$の円周は$2\pi r$だから\\
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
　$\mbox{1周の角}(360^{\circ})=\bunsuu{2\pi r}{r}=2\pi$
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
$\mbox{半周の角}(180^{\circ})=\pi$
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
比なので単位はない（$\sin$などと同じ）\\
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
　度と区別するときは，ラジアン(rad)を付ける
}%
\end{itemize}
%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%


\sameslide

\vspace*{18mm}

\hypertarget{para1pg2}{}

\begin{layer}{120}{0}
\putnotew{96}{73}{\hyperlink{para0pg5}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut||\!\lhd$}}}}}
\putnotew{101}{73}{\hyperlink{para1pg1}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut|\!\lhd$}}}}}
\putnotew{108}{73}{\hyperlink{para1pg1}{\fbox{\Ctab{4.5mm}{\scalebox{1}{\scriptsize $\mathstrut\!\!\lhd\!\!$}}}}}
\putnotew{115}{73}{\hyperlink{para1pg3}{\fbox{\Ctab{4.5mm}{\scalebox{1}{\scriptsize $\mathstrut\!\rhd\!$}}}}}
\putnotew{120}{73}{\hyperlink{para1pg5}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut \!\rhd\!\!|$}}}}}
\putnotew{125}{73}{\hyperlink{para2pg1}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut \!\rhd\!\!||$}}}}}
\putnotee{126}{73}{\scriptsize\color{blue} 2/5}
\end{layer}

\slidepage

\begin{layer}{120}{0}
\putnotese{87}{10}{\input{fig/radian.tex}}
\end{layer}

\begin{itemize}
\item
弧の長さ$\ell$と半径$r$の比\ $\theta(\rad)=\bunsuu{\ell}{r}$\vspace{-1mm}
\item
半径$r$の円周は$2\pi r$だから\\
　$\mbox{1周の角}(360^{\circ})=\bunsuu{2\pi r}{r}=2\pi$
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
$\mbox{半周の角}(180^{\circ})=\pi$
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
比なので単位はない（$\sin$などと同じ）\\
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
　度と区別するときは，ラジアン(rad)を付ける
}%
\end{itemize}

\sameslide

\vspace*{18mm}

\hypertarget{para1pg3}{}

\begin{layer}{120}{0}
\putnotew{96}{73}{\hyperlink{para0pg5}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut||\!\lhd$}}}}}
\putnotew{101}{73}{\hyperlink{para1pg1}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut|\!\lhd$}}}}}
\putnotew{108}{73}{\hyperlink{para1pg2}{\fbox{\Ctab{4.5mm}{\scalebox{1}{\scriptsize $\mathstrut\!\!\lhd\!\!$}}}}}
\putnotew{115}{73}{\hyperlink{para1pg4}{\fbox{\Ctab{4.5mm}{\scalebox{1}{\scriptsize $\mathstrut\!\rhd\!$}}}}}
\putnotew{120}{73}{\hyperlink{para1pg5}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut \!\rhd\!\!|$}}}}}
\putnotew{125}{73}{\hyperlink{para2pg1}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut \!\rhd\!\!||$}}}}}
\putnotee{126}{73}{\scriptsize\color{blue} 3/5}
\end{layer}

\slidepage

\begin{layer}{120}{0}
\putnotese{87}{10}{\input{fig/radian.tex}}
\end{layer}

\begin{itemize}
\item
弧の長さ$\ell$と半径$r$の比\ $\theta(\rad)=\bunsuu{\ell}{r}$\vspace{-1mm}
\item
半径$r$の円周は$2\pi r$だから\\
　$\mbox{1周の角}(360^{\circ})=\bunsuu{2\pi r}{r}=2\pi$
\item
$\mbox{半周の角}(180^{\circ})=\pi$
{\color[cmyk]{\thin,\thin,\thin,\thin}%
\item
比なので単位はない（$\sin$などと同じ）\\
}%
{\color[cmyk]{\thin,\thin,\thin,\thin}%
　度と区別するときは，ラジアン(rad)を付ける
}%
\end{itemize}

\sameslide

\vspace*{18mm}

\hypertarget{para1pg4}{}

\begin{layer}{120}{0}
\putnotew{96}{73}{\hyperlink{para0pg5}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut||\!\lhd$}}}}}
\putnotew{101}{73}{\hyperlink{para1pg1}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut|\!\lhd$}}}}}
\putnotew{108}{73}{\hyperlink{para1pg3}{\fbox{\Ctab{4.5mm}{\scalebox{1}{\scriptsize $\mathstrut\!\!\lhd\!\!$}}}}}
\putnotew{115}{73}{\hyperlink{para1pg5}{\fbox{\Ctab{4.5mm}{\scalebox{1}{\scriptsize $\mathstrut\!\rhd\!$}}}}}
\putnotew{120}{73}{\hyperlink{para1pg5}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut \!\rhd\!\!|$}}}}}
\putnotew{125}{73}{\hyperlink{para2pg1}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut \!\rhd\!\!||$}}}}}
\putnotee{126}{73}{\scriptsize\color{blue} 4/5}
\end{layer}

\slidepage

\begin{layer}{120}{0}
\putnotese{87}{10}{\input{fig/radian.tex}}
\end{layer}

\begin{itemize}
\item
弧の長さ$\ell$と半径$r$の比\ $\theta(\rad)=\bunsuu{\ell}{r}$\vspace{-1mm}
\item
半径$r$の円周は$2\pi r$だから\\
　$\mbox{1周の角}(360^{\circ})=\bunsuu{2\pi r}{r}=2\pi$
\item
$\mbox{半周の角}(180^{\circ})=\pi$
\item
比なので単位はない（$\sin$などと同じ）\\
{\color[cmyk]{\thin,\thin,\thin,\thin}%
　度と区別するときは，ラジアン(rad)を付ける
}%
\end{itemize}

\sameslide

\vspace*{18mm}

\hypertarget{para1pg5}{}

\begin{layer}{120}{0}
\putnotew{96}{73}{\hyperlink{para0pg5}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut||\!\lhd$}}}}}
\putnotew{101}{73}{\hyperlink{para1pg1}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut|\!\lhd$}}}}}
\putnotew{108}{73}{\hyperlink{para1pg4}{\fbox{\Ctab{4.5mm}{\scalebox{1}{\scriptsize $\mathstrut\!\!\lhd\!\!$}}}}}
\putnotew{115}{73}{\hyperlink{para1pg5}{\fbox{\Ctab{4.5mm}{\scalebox{1}{\scriptsize $\mathstrut\!\rhd\!$}}}}}
\putnotew{120}{73}{\hyperlink{para1pg5}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut \!\rhd\!\!|$}}}}}
\putnotew{125}{73}{\hyperlink{para2pg1}{\fbox{\Ctab{2.5mm}{\scalebox{1}{\scriptsize $\mathstrut \!\rhd\!\!||$}}}}}
\putnotee{126}{73}{\scriptsize\color{blue} 5/5}
\end{layer}

\slidepage

\begin{layer}{120}{0}
\putnotese{87}{10}{\input{fig/radian.tex}}
\end{layer}

\begin{itemize}
\item
弧の長さ$\ell$と半径$r$の比\ $\theta(\rad)=\bunsuu{\ell}{r}$\vspace{-1mm}
\item
半径$r$の円周は$2\pi r$だから\\
　$\mbox{1周の角}(360^{\circ})=\bunsuu{2\pi r}{r}=2\pi$
\item
$\mbox{半周の角}(180^{\circ})=\pi$
\item
比なので単位はない（$\sin$などと同じ）\\
　度と区別するときは，ラジアン(rad)を付ける
\end{itemize}

\newslide{弧度法による角度の例}

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
$\deg{60}$は$\deg{180}$の\hakom{\bunsuu{1}{3}}，したがって　$\deg{60}=\hakom{\bunsuu{\pi}{3}}$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
$\deg{60}$は$\deg{180}$の\hakoma{\bunsuu{1}{3}}，したがって　$\deg{60}=\hakom{\bunsuu{\pi}{3}}$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
$\deg{60}$は$\deg{180}$の\hakoma{\bunsuu{1}{3}}，したがって　$\deg{60}=\hakoma{\bunsuu{\pi}{3}}$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
$\deg{60}$は$\deg{180}$の\hakoma{\bunsuu{1}{3}}，したがって　$\deg{60}=\hakoma{\bunsuu{\pi}{3}}$
\item
$\deg{90}$は$\deg{180}$の\hakom{\bunsuu{1}{2}}，したがって　$\deg{90}=\hakom{\bunsuu{\pi}{2}}$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
$\deg{60}$は$\deg{180}$の\hakoma{\bunsuu{1}{3}}，したがって　$\deg{60}=\hakoma{\bunsuu{\pi}{3}}$
\item
$\deg{90}$は$\deg{180}$の\hakoma{\bunsuu{1}{2}}，したがって　$\deg{90}=\hakom{\bunsuu{\pi}{2}}$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
$\deg{60}$は$\deg{180}$の\hakoma{\bunsuu{1}{3}}，したがって　$\deg{60}=\hakoma{\bunsuu{\pi}{3}}$
\item
$\deg{90}$は$\deg{180}$の\hakoma{\bunsuu{1}{2}}，したがって　$\deg{90}=\hakoma{\bunsuu{\pi}{2}}$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
$\deg{60}$は$\deg{180}$の\hakoma{\bunsuu{1}{3}}，したがって　$\deg{60}=\hakoma{\bunsuu{\pi}{3}}$
\item
$\deg{90}$は$\deg{180}$の\hakoma{\bunsuu{1}{2}}，したがって　$\deg{90}=\hakoma{\bunsuu{\pi}{2}}$
\item
$1\degree$は$\bunsuu{\pi}{180}$　　$1$(ラジアン)は$\bunsuu{180}{\pi}$
\end{itemize}

\sameslide

\vspace*{18mm}

\slidepage
\begin{itemize}
\item
$\deg{60}$は$\deg{180}$の\hakoma{\bunsuu{1}{3}}，したがって　$\deg{60}=\hakoma{\bunsuu{\pi}{3}}$
\item
$\deg{90}$は$\deg{180}$の\hakoma{\bunsuu{1}{2}}，したがって　$\deg{90}=\hakoma{\bunsuu{\pi}{2}}$
\item
$1\degree$は$\bunsuu{\pi}{180}$　　$1$(ラジアン)は$\bunsuu{180}{\pi}$
\item
[課題]\monban $\degree$をラジアン，ラジアンを$\degree$に変換せよ．\seteda{30}\\
\eda{$30\degree$}\eda{$45\degree$}
\eda{$\pi$}\eda{$\bunsuu{2\pi}{3}$}
\end{itemize}
\label{pageend}\mbox{}

\end{document}
