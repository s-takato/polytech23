...
\begin{layer}{120}{0}
\putnotese{0}{0}{\input{fig/slide0417.tex}}
\end{layer}
...
\hfill（あまり使わない）
\item
\Ltab{6zw}{掛け算}$ab\ \Longrightarrow$\ \verb|ab|
\item
...
\begin{itemize}
\item
練習-1 KeTMath数式を2次元数式で表示しよう．\seteda{55}\\
...
\eda{3sq(6)}\eda{pir\^{}\!(2)}
\item
解答\seteda{55}\vspace{2mm}\\
\eda{$\dfrac{1+4}{3}$}\eda{$a+\dfrac{b}{c}+d$}\vspace{2mm}\\
\eda{$3\sqrt{6}$}\eda{$\pi r^2$}
\end{itemize}

\newslide{GCとKeTMathによるやりとり}
...
\begin{itemize}
\item
課題 \monban 次の数式をKeTMath数式で書け．\vspace{2mm}\seteda{55}\\
...
\begin{layer}{120}{0}
\putnotese{70}{14}{\input{fig/presen22101rope.tex}}
\end{layer}
